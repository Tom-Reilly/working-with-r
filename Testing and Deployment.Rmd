...
The Rocker project (https://rocker-project.org/) is an R for Docker project that has created a number of preset images for use with R. rocker/shiny and rocker/shiny-verse are preset Docker images for deploying Shiny apps, the only difference being that tocker/shiny-verse also has the `tidyverse` packages installed. Unfortunately it is not feasible to simply run rocker/shiny to deploy Shiny Apps without setting the proxy. Further details on how to set this up can be found here: https://rocker-project.org/use/networking.html.
